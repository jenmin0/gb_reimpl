...
t_data = pickle.load(open("t_data_four", "rb"))
...
class Roberttinyllamataset(TorchDataset):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ghostbuster_only_tinyllama", action="store_true")
    ...
    def get_roberta_predictions(data, train, test, domain):
        print(f"Loading model ghostbuster/roberta/models/roberta_{domain}...")

        roberta_model = RobertaForSequenceClassification.from_pretrained(
            f"ghostbuster/roberta/models/roberta_{domain}", num_labels=2
        )
    ...
    if args.ghostbuster_only_tinyllama or args.ghostbuster:
        run_experiment(
            best_features_map["best_features_only_tinyllama"],
            "Ghostbuster (N-Gram and tinyllama)",
            train_ghostbuster,
        )
    ...
    if args.ghostbuster_other_eval:
        ...
        roberta_model = RobertaForSequenceClassification.from_pretrained(
            f"ghostbuster/roberta/models/roberta_gpt", num_labels=2
        )
        roberta_model.to(device)
        ...
        for dataset in ["lang8"]:
            ...
            exp_data = get_data(gen_fn, ["llama-logprobs s-avg"])

            model_p = LogisticRegression()
            model_p.fit(
                exp_to_data["llama-logprobs s-avg"][
            ...
            # Evaluate roberta
            roberta_test = Roberttinyllamataset(
        ...
        exp_data = get_data(gen_ets, ["llama-logprobs s-avg"])
    ...
    if args.toefl:
        ...
        perplxity_data = get_featurized_data(["llama-logprobs s-avg"], gpt_only=True)
        ...
        toefl_data = get_data(toefl, ["llama-logprobs s-avg"])
