...
def run_roberta_model(model_name, texts, labels):
    roberta_model = RobertaForSequenceClassification.from_pretrained(
        f"ghostbuster/roberta/models/roberta_{model_name}", num_labels=2
    ).to(device)
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        train_roberta_gen(gen_fn_all, "ghostbuster/roberta/models/roberta_gpt", indices=train)
        train_roberta_gen(gen_fn_wp, "ghostbuster/roberta/models/roberta_wp")
        train_roberta_gen(gen_fn_reuter, "ghostbuster/roberta/models/roberta_reuter")
        train_roberta_gen(gen_fn_essay, "ghostbuster/roberta/models/roberta_essay")
        ...
        for domain in ["wp", "reuter", "essay"]:
            results_table.append(
                [
                    ...
                    f"In-Domain ({domain})",
                    *run_roberta_model(
                        "gpt" if domain != "reuter" else "reuter",
