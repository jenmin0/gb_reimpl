...
def score_ngram(doc, model, tokenizer, n=3, strip_first=False):
    ...
    for i in ngrams((n - 1) * [50256] + tokenizer(doc.strip(), add_special_tokens = False), n):
...
def convert_file_to_logprob_file(file_name, model):
    ...
    Example: convert_file_to_logprob_file("data/test.txt", "llama") = "data/logprobs/test-llama.txt"
    """
...
def t_featurize_logprobs(llama_logprobs, tinyllama_logprobs, tokens):
    X = []

    outliers = []
    for logprob in llama_logprobs:
        if logprob > 3:
    ...
    diffs = sorted(llama_logprobs - tinyllama_logprobs, reverse=True)
...
def t_featurize(file, num_tokens=2046):
    """
    Manually handcrafted features for classification.
    """
    llama_file = convert_file_to_logprob_file(file, "llama")
    tinyllama_file = convert_file_to_logprob_file(file, "tinyllama")

    llama_logprobs = get_logprobs(llama_file)[:num_tokens]
    tinyllama_logprobs = get_logprobs(tinyllama_file)[:num_tokens]
    tokens = get_tokens(llama_file)[:num_tokens]

    return t_featurize_logprobs(llama_logprobs, tinyllama_logprobs, tokens)
