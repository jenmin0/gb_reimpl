...
with open("ghostbuster/results/best_features_four.txt") as f:
    best_features = f.read().strip().split("\n")

print("Loading trigram model...")
trigram_model = pickle.load(
    open("ghostbuster/model/trigram_model.pkl", "rb"), pickle.HIGHEST_PROTOCOL
)
model_name = "meta-llama/Llama-2-7b-hf" 
tokenizer = AutoTokenizer.from_pretrained(model_name).encode
wp_dataset = [
...
def get_featurized_data(generate_dataset_fn, best_features):
    ...
    llama, tinyllama, trigram, unigram = get_all_logprobs(
        generate_dataset_fn, trigram=trigram_model, tokenizer=tokenizer
    )

    vector_map = {
        "llama-logprobs": lambda file: llama[file],
        "tinyllama-logprobs": lambda file: tinyllama[file],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--perform_feature_selection_only_tinyllama", action="store_true")
    ...
    if args.generate_symbolic_data_four:
        ...
        t_data = generate_dataset_fn(t_featurize)
        pickle.dump(t_data, open("t_data_four", "wb"))
    ...
    print("Train/Test Split", train, test) # [5728 2154 2406 ... 3436 4256 5801] [4871 5373 5974 ... 1653 2607 2732]
    print("Train Size:", len(train), "Valid Size:", len(test)) # 4795 1199
    print(f"Positive Labels: {sum(labels[indices])}, Total Labels: {len(indices)}") # 3000 6000
    ...
    if args.perform_feature_selection:
        ...
        with open("ghostbuster/results/best_features_three.txt", "w") as f:
    ...
    if args.perform_feature_selection_two:
        ...
        with open("ghostbuster/results/best_features_two.txt", "w") as f:
            for feat in best_features:
                f.write(feat + "\n")
    ...
    if args.perform_feature_selection_one:
        ...
        with open("ghostbuster/results/best_features_one.txt", "w") as f:
            for feat in best_features:
                f.write(feat + "\n")
    ...
    if args.perform_feature_selection_four:
        ...
        with open("ghostbuster/results/best_features_four.txt", "w") as f:
            for feat in best_features:
                f.write(feat + "\n")
    ...
    if args.perform_feature_selection_no_gpt:
        ...
        exp_to_data = {}
        for key in old_exp_to_data:
            if "tinyllama" not in key and "llama" not in key:
                exp_to_data[key] = old_exp_to_data[key]
        ...
        with open("ghostbuster/results/best_features_no_gpt.txt", "w") as f:
            for feat in best_features:
                f.write(feat + "\n")

    if args.perform_feature_selection_only_tinyllama:
        old_exp_to_data = pickle.load(open("symbolic_data_gpt", "rb"))
        ...
        with open("ghostbuster/results/best_features_only_tinyllama.txt", "w") as f:
            for feat in best_features:
                f.write(feat + "\n")
    ...
    if args.perform_feature_selection_domain:
        ...
        with open("ghostbuster/results/best_features_wp.txt", "w") as f:
        ...
        with open("ghostbuster/results/best_features_reuter.txt", "w") as f:
        ...
        with open("ghostbuster/results/best_features_essay.txt", "w") as f:
    ...
    if args.train_on_all_data:
        ...
        with open("ghostbuster/model/features.txt", "w") as f:
            for feat in best_features:
                f.write(feat + "\n")
        pickle.dump(model, open("ghostbuster/model/model", "wb"))
        pickle.dump(mu, open("ghostbuster/model/mu", "wb"))
        pickle.dump(sigma, open("ghostbuster/model/sigma", "wb"))

        print("Saved model to ghostbuster/model/")
    else:
